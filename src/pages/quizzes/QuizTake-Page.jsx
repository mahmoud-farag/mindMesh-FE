    ...
    <div className="pb-10 w-full max-w-5xl mx-auto h-full flex flex-col overflow-hidden space-y-8 px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center justify-between">
          <div className="">
            ...
            <p className="text-slate-500 text-sm mt-1">
              Topic: {quiz.document?.title || 'General Knowledge'}
            </p>
          </div>
          ...
          {currentQuestion.options.map((option, index) => {
            ...
            return (
              <div
                ...
                className={`
                  group relative p-6 rounded-2xl border-2 cursor-pointer transition-all duration-200 shadow-sm hover:scale-105 
                  ${isSelected
              ...
              >
                <div className="flex items-center gap-4">
                  <div
                    className={`
                     size-8  px-2 rounded-full text-sm font-bold border-2 transition-colors flex-center
                      ${isSelected
                  ...
                  {isSelected && (
                    <CheckCircle2 className="w-6 h-6 text-violet-500 ml-auto" />
                  )}
                </div>
              </div>
            );
      ...
      <div className="flex justify-between items-center pt-6 border-t border-slate-100">
        <button
          ...
          className={`text-slate-600 font-medium cursor-pointer border border-slate-300 shadow-md hover:bg-slate-100 active:scale-95 py-2 px-3 sm:py-3 sm:px-8 rounded-xl iFlex-center transition-all duration-200 ${currentQuestionIndex === 0 ? 'opacity-50 cursor-not-allowed' : ''
            }`}
        >
        ...
        >
          {isSubmitting
            ...
            : currentQuestionIndex === totalQuestions - 1
              ? 'Submit'
              : 'Next'}
          {!(isSubmitting || currentQuestionIndex === totalQuestions - 1) && (
